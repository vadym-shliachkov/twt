...
- **Exception — `facts.md` has two sanctioned writers.** When the wiki exists it is the canonical facts ledger (see §2), so the pipeline's fact reconciliation (`twt-curation-define` Step 3.5) writes into it directly alongside the curator. Both merge by fact key and never silently flip a value the other resolved — a disagreement becomes a `CONFLICT` row. No third writer, ever.
