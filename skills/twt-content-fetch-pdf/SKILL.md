...
`<filename>` = the PDF base name, slugified (lowercase, kebab-case). Base output: `.twt-artifacts/pre-design/content/fetched/pdf/<filename>/`. For a folder input, process each PDF into its own `<filename>/` subfolder.
