...
3. **Validate (standalone only):** when NOT in collect mode, dispatch `/twt-audience-validate` (Agent tool) once; read the Scorecard **Band**, **Health**, BLOCKER count. (In collect mode the `fold-validation` dispatch in step 1 already produced the report; standalone never folds, so nothing is overwritten here.)
