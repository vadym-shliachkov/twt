...
**Purpose:** One-call positioning workflow: define → validate in one pass (§9 — no iteration loop).
...
## Step 1 — Detect state
If `positioning.md` exists, ask via the **AskUserQuestion** tool (single-select, header "Pos. state") whether to **Use as-is** (keep the existing document unchanged), **Refine** (address validation findings or update specific sections), **Rebuild** (start over from scratch), or **You decide** (I pick: Refine if a validation-report flags findings, else Use as-is). Pass the choice to the dispatched define skill. Else proceed.
...
1. **Define (subagent):** dispatch `/twt-positioning-define` (Agent tool), **always including `subagent-collect`** (plus the refine/rebuild choice from Step 1 and any answers already gathered). **Only when THIS orchestrator is itself in collect mode, add the token `fold-validation` and fold validation in:** instruct define to self-check against the `/twt-positioning-validate` rubric (§12), write the sibling `validation-report.md` in that format, and record Band/Health + any BLOCKER/WARNING + open decisions in `decisions.md`.
2. **Surface (main thread only):** read `.twt-artifacts/pre-design/positioning/decisions.md`. If `status: open` with entries AND this orchestrator is NOT in collect mode, present each open question / proposed rule via the **AskUserQuestion** tool, collect answers, and re-dispatch define **once** to finalize (`status: resolved`). If this orchestrator IS in collect mode, do NOT ask — merge the child `decisions.md` upward for the parent to surface (nested-subagent bubbling).
3. **Validate (standalone only):** when NOT in collect mode, dispatch `/twt-positioning-validate` (Agent tool) once; read the Scorecard **Band**, **Health**, BLOCKER count. (In collect mode the `fold-validation` dispatch in step 1 already produced the report; standalone never folds, so nothing is overwritten here.)
