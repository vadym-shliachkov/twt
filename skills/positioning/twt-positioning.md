...
description: Orchestrate positioning define/validate with a bounded improvement loop
version: 1.1.1
...
**Purpose:** One-call positioning workflow: define → validate, looping until clean or the bounded loop stops.
...
1. **Define (subagent):** dispatch `/twt-positioning-define` (Agent tool), **always including `subagent-collect`** (plus any user answers from last pass and the refine/rebuild choice from Step 1).
...
3. **Validate (subagent):** dispatch `/twt-positioning-validate` (Agent tool); read the Scorecard **Band**, **Health**, BLOCKER count.
