...
4. None of the above → the source is missing or unrecognized: ask (plain-text prompt) "Give me a Figma frame URL, a live site URL, or the path to a reference image."

Parse the flags:
- **`--name <target-slug>`** — required. If absent, ask (plain-text prompt) for a short slug naming this target (e.g. `homepage-hero`); slugify whatever comes back (lowercase, non-alphanumeric runs → `-`, trim leading/trailing `-`). This is `<target-slug>` in every path below.
...
`measure.mjs` writes `{ "widths": { "<width>": [ ...elements ] , ... }, "how": "<playwright-resolution>" }`; every element it emits already carries `"provenance": "measured"` — that is `measure.mjs`'s own contract, not something this skill adds.
...
**`decisions.md` format** (only written when `collectMode` and at least one breakpoint mapping was auto-decided):
```markdown
---
...
```
After writing it, validate (Bash): `node "${CLAUDE_PLUGIN_ROOT}/tools/check-decisions.mjs" --file ".twt-artifacts/fidelity/<target-slug>/decisions.md"` — fix until it passes; the exact section titles above are what downstream surfacing parses.

## Step 2c — image adapter

`--root` is ignored — the whole image is the root.

Read the image (Read tool). Determine its reference pixel width: if the image's own pixel dimensions are available (many image reads surface them), use that; otherwise ask (plain-text prompt) what pixel width this reference represents, unless the user already stated one (e.g. a single value in `--widths`) — in which case use that instead of asking, per "unless the user states otherwise."
...
```
There is no screenshot to shoot here — the image itself already stands in for `reference/<width>.png`; copy or reference it there for `/twt-fidelity`'s pixel-diff step to use as the reference frame at that width.
...
- Whether a `decisions.md` was written (collect mode only) and what it still has open.
