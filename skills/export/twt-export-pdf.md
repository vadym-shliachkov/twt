...
```powershell
node tools/export-document.mjs --format pdf --input "<markdown-path>" --template "<template-path>"
```

If `--force` was requested:

```powershell
node tools/export-document.mjs --format pdf --input "<markdown-path>" --template "<template-path>" --force
```
