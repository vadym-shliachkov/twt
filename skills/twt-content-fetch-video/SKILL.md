...
surface: command
category: content
description: (v1.0.2) Transcribe a video or audio file (URL or local path) into timestamped Markdown, verbatim or as a full descriptive (accessible) transcript
version: 1.0.2
model: sonnet
...
| Speaker identification, especially with several speakers | turn candidates + naming evidence (Step 7) | role labels, with the basis stated |
...
## Token budget — never read the whole transcript
An hour of speech is ~10,000 words. `index.md` and `segments.json` are built **by the script**, deterministically.

- **Verbatim runs:** do not read either file back to summarize, verify, or "clean up" the output. Report from the JSON summary the tool prints on stdout. Read the transcript only if the user explicitly asks a question about its content, and then read `index.md` alone — never `segments.json`.
- **Descriptive runs:** you do read the speech, but only ever **one window at a time**, through `slice` (Step 7), which prints a single window and nothing outside it. Never open `index.md`, `segments.json`, `frames.json`, or `captions.json` directly — `outline.json` is the only whole-recording file you read, and it is a per-window digest, not the transcript. View only the frames the current window lists.
...
## Step 4 — Transcribe
Run (substituting the chosen values; drop `--language` for auto-detect, and add `--descriptive` only for descriptive depth):
...
The tool writes `index.md`, `segments.json`, and `_meta.md` and prints a JSON summary (slug, duration, language, model, segment and word counts, warnings, file paths). With `--descriptive` it also writes `media.json` (stream layout), `frames/` + `frames.json` (keyframes on visual change), `captions.json` (the file's own subtitle track, if it has a text-based one), `audio-description.md` (a real description track, transcribed, if the file has one), and `outline.json` — and adds speaker-turn candidates and non-speech spans to `segments.json`.

## Step 5 — Verbatim runs stop here
Report as in Step 9 and finish. Everything below is the descriptive pass.

## Step 6 — Read the plan, not the transcript
...
## Step 7 — Work one window at a time
...
2. **Write the window's section** into `transcript.md` (format in Step 8), appending it immediately rather than holding every window in your head. If a run is interrupted, the finished windows are already on disk and you resume from the next one.
...
Speech is verbatim; the verbatim-only record is in `index.md`. Visual and sound
...
- The files written, with paths (and for descriptive runs, that `index.md` is the verbatim record and `transcript.md` the accessible one)
...
- Every warning the tool reported (empty transcript, low language confidence, a long recording run through a small model, no video stream, bitmap subtitles, a failed extraction), and what to do about each
