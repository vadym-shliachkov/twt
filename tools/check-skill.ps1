    ...
    if (Test-Path $manifest) {
        try {
            ...
            foreach ($p in $json.plugins) {
                ...
                $root = Join-Path $MarketplaceRoot $src
                # plugins/<unit>/ is GENERATED by tools/build-units.mjs and holds a
                # copy of every skill in that unit. Counting those copies made every
                # split-out skill look like a duplicate of itself. The marker file is
                # the same one tools/lib/plugin-roots.mjs keys off.
                if (Test-Path (Join-Path $root '.twt-generated')) { continue }
                $roots += $root
            }
