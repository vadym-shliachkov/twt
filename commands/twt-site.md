...
- **The consolidated write approval** (`/twt-inherit-block-creator` Step 5): the CREATE/MODIFY plan for the host's real source tree, in the `## Proposed rules (confirm before binding)` section. Present the **whole scope at once** — CREATE count, then each MODIFY path with what changes and its line estimate — via **AskUserQuestion** (single-select, header "Changes"): **Approve the whole plan** / **New files only — report the modifications as TODOs** / **Stop** / **You decide** (defers every MODIFY to TODOs). On approve, re-dispatch the wrapper so it re-runs the builder with `modifications-approved`. This is the user's own stated requirement for this target — *explain the whole scope at once, don't ask many times* — so it is asked **once per builder run and never per file**. If both gates and the pilot gate below are open at the same time, present them in one pass, not three round trips.
