...
**Then, if `.project-wiki/` exists** (Glob — never a shell command), offer to curate what the run captured. Every phase auto-harvested its decisions into `.project-wiki/inbox.md` (the phase reports carried the pending counts), but capture is not memory until it is curated (§17 — orchestrators may *offer* to curate; they never do it silently). **Interactive mode:** ask via **AskUserQuestion** (single-select, header "Wiki curation"):
- **Curate now** (recommended — promotes this run's captured decisions into cited wiki pages while the context is fresh)
- **Later** (the inbox keeps everything; run `/twt-wiki` when convenient)
- **You decide**

On **Curate now** (or **You decide** resolving to it), dispatch `/twt-wiki` (Agent tool) with no arguments — it curates everything pending and validates; carry its summary (pages created, contradictions raised, health/band) into the report below. **Auto mode / unattended:** never curate silently — skip the question and state the pending inbox count in the report instead. If `.project-wiki/` does not exist, skip this entirely (the wiki is opt-in).
