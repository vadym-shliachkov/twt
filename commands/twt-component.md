...
name: twt-component
category: component
description: (v1.1.2) Orchestrate component define/validate in a single define→validate pass
version: 1.1.2
...
**Purpose:** One-call component workflow: define → validate in one pass (§9 — no iteration loop).
...
3. **Validate (standalone only):** when NOT in collect mode, dispatch `/twt-component-validate` once; read the Scorecard **Band**, **Health**, BLOCKER count. (In collect mode the Step-1 fold-in already produced the report.)
