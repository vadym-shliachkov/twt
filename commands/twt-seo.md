...
name: twt-seo
category: seo
description: (v1.0.1) Orchestrate the SEO define/validate skills in a single define→validate pass
version: 1.0.1
...
3. **Validate (standalone only):** when NOT in collect mode, dispatch `/twt-seo-validate` (Agent tool) once; read the Scorecard **Band**, **Health**, BLOCKER count. (In collect mode the Step-1 fold-in already produced the report.)
