...
- **Child's report states neither shape plainly** (a raw error, a crash, a truncated or ambiguous return) — **hard stop, regardless of iteration.** Do not proceed to Step 5, do not Glob or Read `summary.json`, and do not fall back to the previous pass's numbers. Relay whatever the child did report verbatim, and say plainly in Step 6: iteration `<iteration>`'s measurement could not be confirmed, so the loop stopped rather than continuing on stale data from the prior iteration.
...
- Band / Health (informational only — never the gate; see Step 5): <band> / <health, or "not assessed"> — **omit this line entirely when the stop reason is NOT VERIFIED**; Step 7's path never writes a `summary.json`, so there is no Band/Health to report, not even as "not assessed"
