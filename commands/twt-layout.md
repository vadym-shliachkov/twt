...
name: twt-layout
category: layout
description: (v1.1.2) Orchestrate layout define/validate in a single define→validate pass
version: 1.1.2
...
**Purpose:** One-call layout workflow: define → validate in one pass (§9 — no iteration loop).
...
3. **Validate (standalone only):** when NOT in collect mode, dispatch `/twt-layout-validate` once; read the Scorecard **Band**, **Health**, BLOCKER count. (In collect mode the Step-1 fold-in already produced the report.)
