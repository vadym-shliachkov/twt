...
name: twt-export
category: export
description: (v1.1.2) Orchestrate PDF, DOCX, PPTX, and theme-based exports
version: 1.1.2
...
Do not run Pandoc or conversion commands directly from this orchestrator. Dispatch the matching child skill with the resolved source and theme. _The export children are script wrappers — if your Agent tool supports a `model` parameter, dispatch them on a fast/economical model (e.g. `haiku`)._
