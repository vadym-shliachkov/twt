...
description: (v1.1.3) Orchestrate positioning define/validate in a single define→validate pass
version: 1.1.3
...
3. **Validate (standalone only):** when NOT in collect mode, dispatch `/twt-positioning-validate` (Agent tool) once; read the Scorecard **Band**, **Health**, BLOCKER count. (In collect mode the Step-1 fold-in already produced the report.)
