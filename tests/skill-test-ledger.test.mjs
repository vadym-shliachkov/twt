...
const newRun = () => {
  ...
  initRun(dir, { skill: 'twt-demo', criteriaHash: 'sha256:abc', scope: ['contract'], target: 'C:/t', startTreeClean: true, selfDeclared: ['C-002'] });
  return dir;
};
...
test('converged: all PASS with a non-self-declared criterion is converged-pass', () => {
  const run = { selfDeclared: ['C-002'], iterations: [{ n: 1, verdicts: { 'C-001': 'PASS', 'C-002': 'PASS' } }] };
...
test('converged: all PASS but every passing criterion is self-declared is converged-pass-weak', () => {
  const run = { selfDeclared: ['C-002'], iterations: [{ n: 1, verdicts: { 'C-002': 'PASS' } }] };
  assert.equal(converged(run), 'converged-pass-weak');
});

test('converged: UNVERIFIABLE counts as not-passing', () => {
  const run = { selfDeclared: [], iterations: [{ n: 1, verdicts: { 'C-001': 'UNVERIFIABLE' } }] };
...
test('converged: an identical verdict map two iterations running is no-progress', () => {
  ...
  const run = { selfDeclared: [], iterations: [{ n: 1, verdicts: v }, { n: 2, verdicts: { ...v } }] };
  assert.equal(converged(run), 'no-progress');
});

test('converged: no-progress wins over iteration-cap when both apply', () => {
  const v = { 'C-001': 'FAIL' };
  const run = { selfDeclared: [], iterations: [{ n: 1, verdicts: { ...v } }, { n: 2, verdicts: { ...v } }, { n: 3, verdicts: { ...v } }] };
  assert.equal(converged(run), 'no-progress');
});

test('converged: cap reached with changing verdicts is iteration-cap', () => {
  const run = { selfDeclared: [], iterations: [
...
test('converged: a run with no valid iterations continues', () => {
  assert.equal(converged({ selfDeclared: [], iterations: [] }), 'continue');
});
