...
- **Build target:** Static HTML vs. Elementor vs. this project's existing stack (inherit)
...
**Inherit-target surfacing (interactive only, during Development).** When `<target>` = **inherit**, two user-facing gates live inside the phase wrapper you dispatched, and because that wrapper runs with `subagent-collect` it cannot ask them itself — **you are the surfacing point** (§13). Both arrive in `.twt-artifacts/inherited/decisions.md` and in the wrapper's returned report:
- **The inherited-conventions review gate** (`/twt-inherit-define` Step 7, surfaced up through `/twt-develop` Step 3 or `/twt-site-dev` Step 3): the detected stack, styling system, component idiom, routing, asset root and exemplars that everything the build produces will bind to. Present its open questions via **AskUserQuestion** (each with a **"You decide"** accepting the recorded `model-leaning`), then re-dispatch the wrapper with the answers so the conventions are rewritten before any page is built.
- **The consolidated write approval** (`/twt-inherit-block-creator` Step 5): the CREATE/MODIFY plan for the host's real source tree, in the `## Proposed rules (confirm before binding)` section. Present the **whole scope at once** — CREATE count, then each MODIFY path with what changes and its line estimate — via **AskUserQuestion** (single-select, header "Changes"): **Approve the whole plan** / **New files only — report the modifications as TODOs** / **Stop** / **You decide** (defers every MODIFY to TODOs). This is the user's own stated requirement for this target — *explain the whole scope at once, don't ask many times* — so it is asked **once per builder run and never per file**.

  **On approve, re-dispatch the wrapper with `modifications-approved`** (plus `subagent-collect` and `--target inherit` as always). `/twt-develop` handles that token in its Step 4 *Continuation tokens* rule: it reads the open plan from `.twt-artifacts/inherited/decisions.md`, re-runs the builder for that page with the approval, and marks the decision resolved. Do not invent your own token — `modifications-approved` is the one `/twt-develop` receives.

  **If the write approval and the pilot gate below are both open, present them in one pass — and then re-dispatch with BOTH tokens, `modifications-approved pilot-approved`, in one call.** They compose deliberately: `/twt-develop` applies the approved writes first and only then lets `pilot-approved` skip ahead to the remaining pages. Never send `pilot-approved` alone after approving a write plan — on its own it skips Step 4a entirely and the pilot page's approved modifications are silently never applied. If the conventions gate is open too, that one is resolved on its own earlier re-dispatch (it must be settled before any page is built), so at most two tokens ever travel together.
- **Auto mode never auto-approves a MODIFY.** It may resolve the conventions questions from context (logged as auto-decisions), but every modification to a file the host already owns stays a TODO. Silence is not consent for edits to a repo the user did not hand us.

**Pilot-page surfacing (interactive only, during Development via `/twt-develop`).** A full multi-page build is the most expensive part of the run, so check one page before committing to all. When Development builds via `/twt-develop` (not Figma express) and this is **not** auto mode: `/twt-develop` (dispatched with `subagent-collect`) builds only the **pilot page** and returns an open "Pilot page built — approve to build the rest" decision in `.twt-artifacts/<html-site|elementor-theme|inherited>/decisions.md` with the pilot's path and the list of remaining pages. Read it and present the gate to the user via the **AskUserQuestion** tool (single-select, header "Pilot"): **Build the remaining N pages** / **Add one more pilot page** / **Adjust the pilot** / **Stop here**. On approve, re-dispatch `/twt-develop --target <target> pilot-approved` (with `subagent-collect`) to promote the rest — **for `<target>` = inherit, if the builder's write plan was surfaced and approved in the same pass, send `modifications-approved pilot-approved` together per the inherit clause above; `pilot-approved` alone skips Step 4a and the approved writes are never applied.** On adjust/add, forward the feedback and re-dispatch, then re-surface; on stop, leave the remaining pages unbuilt and record it. Log each gate Q&A to the Timeline. **Auto mode skips this** — dispatch `/twt-develop auto …` so it builds all pages in one pass.
